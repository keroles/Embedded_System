...
void LCD_display_real_number (double  Number )
{
	...
	/*if (Number<0)
	{
		...
		LCD_Send_A_Character(sign);
	}*/
	
	int Tint1 = Number ;
	float fraction = (Number-Tint1);
	LCD_display_number(Tint1);
	LCD_Send_A_Character('.');
	for (i=0;i<3;i++)
	{
		if (fraction*pow(10,(i+1))<1)
		{ 
	...
	LCD_display_number( fraction*10000);
