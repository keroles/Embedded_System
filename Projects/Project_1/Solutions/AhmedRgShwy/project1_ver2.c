...
U8 addPacket( CAN **const mPtr, U8 *id, U8 *n  );
U8 managePackets( CAN **const mPtr );
U8 deletePacket( CAN **const mPtr, U8 ID, U8 num );

int main(void)
{
	U8 num, idv, check= 1 ;
	CAN can, *canPtr ;
	CAN *IDV_ptr[IDV_NUM]={NULL} ;
	CAN **mainPtr= IDV_ptr ;
...
{
	volatile U8 check= 1;
	U8 ch, num=0, id2=0, id= 0, idv_index= 0, i= 0 ;
	CAN tempCAN ;
	...
	while(1)
	{
		...
		/**************************************************************************************/
			...
			else
			{
				...
				printf("Packet number-%d has been changed, form ID_type-%d\n", num,tempCAN.IDv );
			}
			break;			
		/**************************************************************************************/
			case 'V': case 'v':
		...
		/**************************************************************************************/
			case 'N': case 'n':
			printf("Packets number are %d, and are distributed as: \n",
			...
			printf("packets for ID-80/Garage are: %10d\n", IDV_packetsNum[IDV80_INDEX]);
			printf("packets for ID-110/Fridge are: %9d\n", IDV_packetsNum[IDV110_INDEX]);
			break;
		/**************************************************************************************/
			case 'T': case 't':
	...
	if( check )
	{
		*id= can.IDv;
		...
		if( !IDV_packetsNum[idv_index] )
		{
			...
			{
				#if DEBUGING
				printf("<<DEBUGING: One packet dynamic size reserved at address%d.>>:\n",
				mPtr[idv_index]);
				#endif
		...
		else
		{
			...
			{
				#if DEBUGING
				printf("<<DEBUGING: Dynamic size resized by one packet at address%d.>>\n", tempPtr[IDV_packetsNum[idv_index]]);
				#endif
				free(mPtr[idv_index]);
				#if DEBUGING
